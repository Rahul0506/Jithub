...
def pass_to_classifier(dataset, db):
    '''
    ...
    for tuple_index in range(numEntries):
        ...
        output = predict(text_clf, classifier_input)
    ...
    for list_index in range(numEntries):
        ...
        insert_command = "update %s set classification=%d where input_text=%s" %(db_keys[1], output2, corr_input)
